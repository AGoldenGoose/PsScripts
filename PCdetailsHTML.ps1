...
{    
    ...
    #Build the object file, the data from which is imported to the HTML file.
 

$tblObject = New-Object PSObject -property  @{
...
"@
    ...
    $Pre = "<h1>PC Details Report</h1>"
    $Post = "</p></h2>Ran on $(Get-Date -f 'MM/dd/yyyy hh:mm:ss tt')</body></html>"

    #Export the fields you want from above in the specified order
$tblObject | Select PCName, Manufacturer, Version , Ram, CPU, GPU, User , HDDSizeGB , HDDSpace , OS, SerialNumber , IP , Mac  | ConvertTo-Html -PreContent $Pre -PostContent $Post -Head $Header | Out-File \\christie\dfsroot$\homedrives1\George.Brophy\PSReporting\pcdetails.html
